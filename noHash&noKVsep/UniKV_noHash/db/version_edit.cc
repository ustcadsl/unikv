...
void VersionEdit::EncodeTo(std::string* dst) const {
  ...
  for(int k=0;k<partition_InfoVec.size();k++){
    ...
    /*PutVarint32(dst,  kReplayNumber);
    PutVarint32(dst, NoPersistentNum);*/
}
    ...
    switch (tag) {
      ...
      case kDeletedFile:{
          ...
          if (GetLevel(&input, &level) &&
            ...
            //printf("  delete partition:%d,level:%d,fileID:%d\n",partition,level,number);
      ...
      case kNewFile:
	    ...
	    GetVarint32(&input, &f.partition)&& //
          ...
          //printf("  partition:%d,level:%d,fileID:%d\n",f.partition,level,f.number);
      ...
      case kPartitionInfo:
        ...
        break;
      /* case kReplayNumber:
      if (GetVarint32(&input, &NoPersistentNum)) {
      //     has_log_number_ = true;
        printf("NoPersistentNum:%u\n",NoPersistentNum);
      }else{
        msg = "new-file entry";
      }
      break;*/
...
std::string VersionEdit::DebugString() const {
 ...
 for(int k=0;k<config::kNumPartition;k++){//
  ...
  for(int k=0;k<config::kNumPartition;k++){
    ...
    for (DeletedFileSet::const_iterator iter = deleted_files_[k].begin(); iter != deleted_files_[k].end(); ++iter) {
      r.append("\n  DeleteFile: ");
