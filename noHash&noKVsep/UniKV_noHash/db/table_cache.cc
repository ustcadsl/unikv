...
Status TableCache::FindTable(uint64_t file_number, uint64_t file_size,
  ...
  *handle = cache_->Lookup(key);//look key in cache
  if(isPrefetch){
  ...
  if (*handle == NULL) {//key not in cache
    //printf("L1 not in page cache=1,prefetch:%d！\n",isPrefetch);
    std::string fname = TableFileName(dbname_, file_number);
    //RandomAccessFile* file = NULL;
    int fileID;
    //PosixRandomAccessFile* file = NULL;
    ...
    } else {//cache KV pair in TableCache
      TableAndFile* tf = new TableAndFile;//
      tf->file = file;
      tf->table = table;
      *handle = cache_->Insert(key, 0,tf, 1, &DeleteEntry);//handle point to the inserted handle
    }
  }
  return s;
}
...
Status TableCache::FindTable(uint64_t file_number, uint64_t file_size,
  ...
  EncodeFixed64(buf, file_number);
  Slice key(buf, sizeof(buf));
  *handle = cache_->Lookup(key);//look key in Table_cache
  if(isPrefetch){
     #ifdef READ_SSTABLE_AHEAD
     std::string fname = TableFileName(dbname_, file_number);
      int fileID;
      ...
      //#ifdef READ_AHEAD
      //int ret=readahead(fileID,0,file_size); 
      int ret=posix_fadvise(fileID,0L,file_size, POSIX_FADV_WILLNEED);//
      //printf("ret=%d,file_size=%lu\n", ret,file_size);
      if(ret == -1){
  ...
  if (*handle == NULL) {///key not in cache
    ...
    if (!s.ok()) {
      ...
      // We do not cache error results so that if the error is transient,
      // or somebody repairs the file, we recover automatically.
    } else {//cache KV pairs in TableCache
      TableAndFile* tf = new TableAndFile;//
      tf->file = file;
      tf->table = table;
      *handle = cache_->Insert(key, 0,tf, 1, &DeleteEntry);//handle point to the inserted handle
  ...
  *handle = cache_->Lookup(key);////look key in Table_cache
  ...
  if (*handle == NULL) {//key not in cache
   // fprintf(stderr, "table %d in L0 doesn't cache Index_Part\n", (int)file_number);
    printf("L0 not in page cache,prefetch:%d！\n",isPrefetch);
    std::string fname = TableFileName(dbname_, file_number);
    //RandomAccessFile* file = NULL;
    int fileID;
    Table* table = NULL;
    ...
    if (!s.ok()) {
      ...
      tf->file = file;
      tf->table = table;
      *handle = cache_->Insert(key,1, tf, 1, &DeleteEntry);//handle point to the inserted handle
    }
  }else{
     *tableCacheNum=*tableCacheNum+1;
  }
...
Status TableCache::Get(const ReadOptions& options,
  ...
  if (s.ok()) {
    ...
    s = t->InternalGet(options, k, arg, saver);//look key in SSTables
...
Status TableCache::Get(const ReadOptions& options,
  ...
  Status s ;
  if(level==0){
    s= FindTableInL0(file_number, file_size, &handle,readDataSizeActual,tableCacheNum,false);//
    if (s.ok()) {
      Table* t = reinterpret_cast<TableAndFile*>(cache_->Value(handle))->table;
      s = t->InternalGetL0Table(options, k, arg, saver,readDataSizeActual,blockCacheNum);//look key in SSTables
      cache_->Release(handle);
    }
  }
  else{
     s = FindTable(file_number, file_size, &handle,readDataSizeActual,tableCacheNum,false);
     if (s.ok()) {
        Table* t = reinterpret_cast<TableAndFile*>(cache_->Value(handle))->table;
        s = t->InternalGet(options, k, arg, saver,readDataSizeActual,blockCacheNum,BFfilter);//look key in SSTables
        cache_->Release(handle);
     }
  }
  return s;
