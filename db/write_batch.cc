...
//////////////////////////////////////////////////////
//int WriteBatch::GetKeyValue(Slice *key) const {
Status WriteBatch::GetKeyValue(Slice *key) const {
  Slice input(rep_);
    ...
    switch (tag) {
      case kTypeValue:
        if (GetLengthPrefixedSlice(&input, &key) &&
          ...
          //handler->Put(key, value, partition);
      ...
      case kTypeValue:
        if (GetLengthPrefixedSlice(&input, &key) &&
          ...
          if(newPartition!=(int)partition){
            //fprintf(stderr,"newPartition:%d, partition:%d\n", newPartition, partition);
...
Status WriteBatchInternal::InsertInto(const WriteBatch* b,
                                      MemTable** memtable) {//Insert MemTable
