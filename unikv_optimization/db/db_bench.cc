...
//fprintf(stderr,"in DoWrite\n");
    ...
    //  fprintf(stderr,"before  db_->Write\n");
      s = db_->Write(write_options_, &batch);
       //fprintf(stderr,"after  db_->Write\n");
      if (!s.ok()) {
...
void ReadRandom(ThreadState* thread) {//////////////////////////////////
    ...
    beginTime=Env::Default()->NowMicros();
  //  g_env->SleepForMicroseconds(10000000);//休眠100ms 
   // reads_=1000000;
    for (int i = 0; i < reads_; i++) {
      char key[100];
      const int k = thread->rand.Next() % FLAGS_num;
      snprintf(key, sizeof(key), "%016d", k);
      double beginop = Env::Default()->NowMicros();
      if (db_->Get(options,Slice(key), &value).ok()) {/////////////////////////////////////ReadRandom
	      bytes += sizeof(key) + value.size();
        found++;
      }
      double endop = Env::Default()->NowMicros();
      double micros = endop - beginop;
      //outf<<micros<<std::endl;
      
      thread->stats.FinishedSingleOp();
    }
    endtime=Env::Default()->NowMicros();
    thread->stats.AddBytes(bytes);
    char msg[100];
   // snprintf(msg, sizeof(msg), "(%d of %d found)", found, num_);
     snprintf(msg, sizeof(msg), "(%d of %d found)", found, reads_);
    thread->stats.AddMessage(msg);
    char ops[100];
      snprintf(ops, sizeof(ops), ";  %11.3f op/s\n", reads_/((endtime-beginTime)*1e-6));
    thread->stats.AddMessage(ops);
  }
