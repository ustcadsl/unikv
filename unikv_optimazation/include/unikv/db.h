...
class DB {
 public:
  ...
  uint64_t readDataSizeActual;
  uint64_t writeDataSize;
  uint64_t writeDataSizeActual;
  uint64_t tableCacheNum;
  uint64_t blockCacheNum;
  
  long int reduceIOInComp, totalIOInComp,totalDropKeyN;
  
  //uint64_t sizeTotal;
  int readMem,readHotMem,readImm,readIn0,readIn1;
  long int cacheRead;
  ...
  //virtual Status InsertRequestQueue(const WriteOptions& options, const Slice& key, const Slice& value) = 0;

  virtual int MapCharKeyToPartition(Slice charKey) = 0;

  virtual void sleepWrites() = 0;
